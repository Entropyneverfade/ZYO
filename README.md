...
| 外部适配器 | HiGHS/Gurobi/COPT | 仅供显式、隔离的独立比较，结果不属于 ZYO 原生能力 |
...
公开包提供独立预期的核心/储能测试及 60 个冻结小整数开发回归。它们不是未见保留集，也不是国际性能排名。完整本地研发记录含设备及其他研究信息，因此不直接公开整个历史仓库；公开源码由原项目白名单导出，求解算法未为发布改写。

已有一个 8760 小时合成储能循环 MILP 的自主运行证据，但其搜索仅一个根节点，不能外推全年机组组合、一般困难 MILP 或全球领先。自由/无限界、退化及不可行检测仍有明确限制，详见能力说明。

许可证为 MIT，保留原始版权声明。提交问题时请使用可公开的小型复现输入，不要上传许可证密钥、个人信息或未获授权的论文数据。
